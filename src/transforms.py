...
class Triangular(Distribution):
    ...
    Inspired by https://github.com/tensorflow/probability/blob/master/tensorflow_probability/python/distributions/triangular.py
    """

    arg_constraints = {
        "low": constraints.real,
        "high": constraints.dependent,
        "peak": constraints.dependent,
    }
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        with torch.no_grad():
            ...
            return torch.where(
                ...
                self.low
                + torch.sqrt(samples * interval_length * (self.peak - self.low)),
                self.high
                - torch.sqrt(
                    (1.0 - samples) * interval_length * (self.high - self.peak)
                ),
            )
    ...
    def log_prob(self, value):
        ...
        result_inside_interval = torch.where(
            (value >= self.low) & (value <= self.peak),
            torch.tensor(2.0).log()
            + torch.log(value - self.low)
            - torch.log(interval_length)
            - torch.log(self.peak - self.low),
            torch.tensor(2.0).log()
            + torch.log(self.high - value)
            - torch.log(interval_length)
            - torch.log(self.high - self.peak),
        )

        return torch.where(
            (value < self.low) | (value > self.high),
            torch.log(torch.ones_like(value) * 1e-6),
